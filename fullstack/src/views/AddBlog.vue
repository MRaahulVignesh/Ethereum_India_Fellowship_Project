    ...
    <b-container variant="dark">
      <b-form @submit="onSubmit" @reset="onReset" v-if="show">
        ...
        <b-form-group id="input-group-2" label="Category" label-for="input-2">
          <b-form-input id="input-2" v-model="form.category" required placeholder="Enter category"></b-form-input>
        </b-form-group>
...
export default {
  ...
  mounted: async function() {
      if (window.ethereum) {
        ...
        blogBackend = new BlogBackend(ContractDetails.abi, ContractDetails.contractAddress, web3 = this.web3);
      },
  methods: {
    onSubmit(evt) {
			evt.preventDefault();
